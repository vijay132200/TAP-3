...
import { CheckCircle, XCircle, Settings, Clock, Brain, FileText, AlertTriangle } from 'lucide-react';
...
export default function ChatInterface({
            ...
            <div className="flex-1">
              <div className="bg-primary/5 border border-primary/20 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-sm font-medium text-primary" data-testid="reasoning-status">Autonomous Reasoning Engine Active</p>
                </div>
                <p className="text-sm text-muted-foreground" data-testid="reasoning-description">
                  Analyzing query parameters and determining optimal tool selection...
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex-1">
              <div className="bg-accent/5 border border-accent/20 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-sm font-medium text-accent" data-testid="knowledge-status">Consulting Proprietary Knowledge Module</p>
                </div>
                <p className="text-sm text-muted-foreground mb-3" data-testid="knowledge-description">
  ...
  const renderApprovalPrompt = (message: Message) => {
    ...
    return (
      <div className="animate-slideIn">
        <div className="bg-warning/10 border-2 border-warning rounded-lg p-5 shadow-lg" data-testid="approval-prompt">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="text-base font-semibold text-warning mb-2" data-testid="approval-title">
                ⚠️ Human-in-the-Loop: Expert Confirmation Required
              </h3>
              <p className="text-sm text-foreground mb-3" data-testid="approval-description">
                The agent has detected a <strong>high-risk scenario</strong> based on your proprietary knowledge rules.
              </p>
              {message.metadata.reasoning && (
                <div className="bg-card rounded-lg p-4 mb-4 space-y-2 text-sm">
                  ...
                  {message.metadata.knowledgeUsed && message.metadata.knowledgeUsed.length > 0 && (
                    <p className="text-accent" data-testid="approval-knowledge">
                      <strong>Knowledge Used:</strong> {message.metadata.knowledgeUsed.join(', ')}
                    </p>
                  )}
                </div>
              )}
              <p className="text-sm text-muted-foreground mb-4" data-testid="approval-waiting">
                The agent is paused and awaiting your explicit approval before proceeding.
              </p>
              ...
              <div className="flex-1">
                <div className="bg-card border border-border rounded-lg p-4 shadow-sm">
                  ...
                  <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <span data-testid="system-intro-time">{formatTime(new Date())}</span>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="flex gap-3 justify-end">
                  <div className="flex-1 max-w-2xl">
                    <div className="bg-primary text-primary-foreground rounded-lg p-4 shadow-sm">
                      <p className="text-sm" data-testid={`user-message-${index}`}>{message.content}</p>
                      <div className="mt-3 flex items-center gap-2 text-xs text-primary-foreground/70 justify-end">
                        <span data-testid={`user-message-time-${index}`}>{formatTime(message.timestamp)}</span>
                      </div>
                    </div>
                  </div>
                        ...
                        {message.metadata?.approvalDecision && (
                          <div className="flex items-center gap-2 mb-3">
                            <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-accent/10 text-accent">
                              Expert {message.metadata.approvalDecision === 'approve' ? 'Approved' : message.metadata.approvalDecision === 'reject' ? 'Rejected' : 'Modified'}
                            </span>
                          </div>
                        )}
